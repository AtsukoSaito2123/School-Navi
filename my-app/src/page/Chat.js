...
const Chat = () => {
    ...
    const addMember = async (e) => {
        e.preventDefault();
        try {
            // userコレクションにデータのあるメンバーだけ、メンバー追加
            ...
            const userDocRef = doc(db, 'users', currentUser.uid);
            const membersCollectionRef = collection(userDocRef, 'MemberList');
            // 自分のメンバーIDと追加するメンバーIDを取得し、配列にして、アルファベット順にソート
            ...
            // 新しいメンバーのデータをusersコレクションから参照してログインしているuserコレクションのmemberListにデータ格納し
            //ログインしているユーザー専用のメンバーリストを表示するようにしてます。
            await Promise.all(memberData.map(async (member) => {
                ...
                // 相手のメンバーリストにこちら側の情報も自動で追加し、チャットルームの共有
                // 他のユーザーのドキュメント内のMemberListコレクションに追加
